...
But as Insulin and Age appear to be is highly insignificant, so we try to convert them to factors to overcome this. and also by seeing the data we can see that Insulin has many Outliers with large values that do not exist practically.
...
```{r}
new_dataset <- diabetes[,-8]
head(new_dataset)
```

From above we have removed the Age from the dataset and created new dataset

```{r}
fit_2 <- glm(Outcome ~ Pregnancies + Glucose + BloodPressure + BMI + 
    DiabetesPedigreeFunction + Age_group, data = diabetes, family = "binomial")
summary(fit_2)
```
