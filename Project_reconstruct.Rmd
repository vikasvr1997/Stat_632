...
fit_2 <- glm(Outcome ~ Pregnancies + Glucose + BloodPressure + BMI + 
    DiabetesPedigreeFunction + Age + Insulin:DiabetesPedigreeFunction, data = diabetes, family = "binomial")
summary(fit_2)
```

```{r}
plot(diabetes$Insulin,diabetes$DiabetesPedigreeFunction)
```
...
# Create train-test split with 70-30 ratio
train_index <- createDataPartition(diab_new$Outcome, p = 0.7, list = FALSE)
train_data <- diab_new[train_index, ]
test_data <- diab_new[-train_index, ]

# Fit logistic regression model on training data
T_T_S_model <- glm(Outcome ~ Glucose + BloodPressure+
                       BMI + DiabetesPedigreeFunction + Pregnancies:Age, 
                     data = train_data, family = binomial)
# Predict on training data 
predictions <- predict(T_T_S_model, newdata = train_data, type = "response")
predicted_classes_train <- ifelse(predictions > 0.5, 1, 0)
...
```{r}
ggplot(diabetes)+
  geom_boxplot(aes(x=Age ,y = Pregnancies , fill = Outcome))+
  labs(x="Age",
       y="Pregnancies",
